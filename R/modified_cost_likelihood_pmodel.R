modified_cost_likelihood_pmodel <- function(
  ...
  if (!identical(sort(c(names(par_calibrated_model), names(par_fixed))), required_param_names)) {
    stop(sprintf(
      paste0(
        "Error: Input calibratable and fixed parameters do not ",
    ...
    if (ncol(obs_flux) < 3) {
      ...
      df_flux <- df |>
        ...
        dplyr::left_join(
          obs_flux,
          by = c("sitename", "date")
        ) # observations with missing date are ignored
    }
  } else {
    df_flux <- data.frame()
  }
  ...
  for (target in targets) {
    # check (needed?):
    if (target %in% colnames(df_flux) & target %in% colnames(df_trait)) {
      stop(
