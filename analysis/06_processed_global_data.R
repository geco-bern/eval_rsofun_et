# This script processed the thre global product used to compare the gRSOFUN global performance

# Here, I take the raw data present in data/archive_project folder and I extract the montlhy sum of ET between 1997 and 2011
...
# Load functions
source(here::here("R/calc_vpd_td.R"))

# ERA5 extraction
####---------

# !!! ERA 5 needs to be added !!!
...
rast_final <- terra::rast(paste0("/data/archive_projects/eval_rsofun_et/PML/data/Monthly_PML_ETa_",1997,".nc"))

names(rast_final) <- seq(date("1997-01-01"),date("1997-12-01"), by = "months")

for(i in years){

 tmp <- terra::rast(paste0("/data/archive_projects/eval_rsofun_et/PML/data/Monthly_PML_Ec_",i,".nc"))
...
saveRDS(montlhy_pml,"/data/archive_projects/eval_rsofun_et/processed_global_data/montlhy_pml.rds")
####---------
#FLUXCOM extraction
####---------

dir <- "/data/archive_projects/eval_rsofun_et/FLUXCOM/ensemble"
...
# STEP1: take mean monthly temp and patm from global product
# # done it in extrc ....

temp_patm <- readRDS("/data/archive_projects/eval_rsofun_et/processed_global_data//montlhy_tmp_patm_for_fluxcom.rds")
...
for(i in 1:180){
  ...
  prova2 <- left_join(prova2 |> mutate(lonlat = paste0(lon,lat)),
                      tmp, by = "lonlat") |>
    rename(le = aet) |>
    mutate(le = le *10^6 / (60 * 60 * 24)) |> # transofrm from MJ day to J s
...
saveRDS(montlhy_fluxcom,"/data/archive_projects/eval_rsofun_et/processed_global_data/montlhy_fluxcom.rds")
####--------------
