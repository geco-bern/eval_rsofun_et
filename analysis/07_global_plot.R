...
lateral_profile <- ggplot(df) +
  ...
  theme_minimal() +
  ylab(expression(paste("Simulated AET (mm yr"^{
    -1
  }, ")"))) +
  xlab("lat") +
  theme(
...
ggplot(p_model2, aes(x = date, y = med)) +
  geom_line()
...
for (i in 1:nrow(fdk_site_info)) {
  ...
  tmp <- data.frame(
    sitename = fdk_site_info$sitename[i],
    date = date(paste0(
      lon_1$year,
...
rmse_plot <- ggplot(rmse, aes(x = model, y = RMSE)) +
  geom_boxplot() +
...
cor_plot <- ggplot(corr, aes(x = model, y = correlation)) +
  geom_boxplot() +
  scale_x_discrete(
    limits = c("P_model", "Pml", "Fluxcom", "ERA5")
  ) +
...
gglegend <- plot_discrete_cbar(
  ...
  legend_title = expression(paste("Simulated AET (mm yr"^{
    -1
  }, ")")),
